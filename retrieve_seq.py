...
def main():
    ...
    parser_splice.add_argument('-f', '--file', action="store", help="A multifasta file",
                                required=False)

    parser_splice.add_argument('-r', '--range', action="store", nargs='+', help="A values in the form start-end space separated. 10-20 50-60 70-100",
                                required=False)

    parser_splice.add_argument('-g', '--gene', action="store", help="GENE  A gene (or chromossome) name",
                                required=False)
    ...
    if hasattr(args, 'range'):  # args.start exists and has a value
        fasta = args.file
        range = args.range
        gene_name = args.gene
